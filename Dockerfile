...
# Install all dependencies (build stages need the dev toolchain)
RUN bun install --frozen-lockfile

# --------------------------------------------------------------------------
# Stage 1b: Production dependencies only
#
# The runtime needs the backend's production dependencies and nothing else: the
# container runs scripts/migrate.ts (bun:sqlite + drizzle-orm) and the backend
# server. It does not compile anything.
#
...
# --------------------------------------------------------------------------
FROM oven/bun:1.3.14-alpine@sha256:5acc90a93e91ff07bf72aa90a7c9f0fa189765aec90b47bdbf2152d2196383c0 AS prod-deps
...
# --ignore-scripts: the root `preinstall` hook runs `only-allow`, which is itself a
# devDependency and so is absent from a production install (exit 127). Nothing in the
# production dependency set needs a lifecycle script - the runtime is pure JS on Bun,
# with no native module to compile.
...
# License notices and downstream guidance. The local verification image includes Bun with
# statically linked LGPL libraries plus GPL/LGPL Alpine packages. Derived projects keep these
# files in any image they choose to distribute and supply their own corresponding source.
...
# Dependencies come from the production-only install, NOT from the build stage. The
# build stage's tree contains the whole dev toolchain; copying it here shipped vite,
# typescript, puppeteer, eslint and friends to production, and esbuild had to be
# hunted down afterwards because its prebuilt Go binary carried CVEs. A production
# install never brings them in the first place, so there is nothing to strip.
COPY --from=prod-deps /app/node_modules/ node_modules/
COPY --from=prod-deps /app/backend/node_modules/ backend/node_modules/

# Remove native build artifacts and test files
