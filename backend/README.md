...
| Config         | TypeBox (for JSON config validation) |
...
├── config/
│   ├── configLoader.ts    # JSON config loader with schema validation
│   ├── configLogger.ts    # Shared environment-aware config logger
│   ├── configSchema.ts    # Root TypeBox schema
│   ├── configSchemas/     # Per-domain TypeBox schemas (security, server, etc.)
