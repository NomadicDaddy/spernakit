 ...
 * location.
 *
 * Historically the template drifted into creating `backend/data/` databases
 * because path resolution derived a DB location from `database.url` without
 * validating where it landed. This module provides a pure resolver +
 * containment assertion shared by the startup config guard (`app.ts`) and the
 * `check:db-location` CI gate, so a misconfigured `database.url`/path fails
 * fast instead of silently recreating the drift.
 */
