...
async function seedUsersIfEmpty(
	...
	// and first-login flows aren't blocked. When ON, the historical secure-by-default policy
	// is preserved. emailVerified is always true so email-verification gates never trip
	// the seeded admin/manager/operator/viewer accounts.
	//
	// History (this value flip-flopped 4+ times before the toggle existed; see commit
	// 063f2eb2 "fix: set requiresPasswordChange to false in seed users" and the
	// 2026-04-04 commit-archaeology report). The settings-driven approach replaces the
	// hard-coded value so the previously-conflicting audit and Docker-crawltest
	// requirements can both be satisfied without further flip-flopping.
