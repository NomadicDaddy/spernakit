...
import { WRITABLE_SETTING_KEYS } from '../../constants/appFeatures.ts';
...
const settingsGeneralRoutes = new Elysia({
	...
	.put(
		...
		({ body, params, set, user }) => {
			...
			if (!WRITABLE_SETTING_KEYS.has(params.key)) {
		...
		},
		{
			...
			detail: {
				description:
					...
					'Only keys in the runtime settings write allowlist (app feature and ' +
					'notification setting keys) are accepted; all other keys are rejected with ' +
					'400. Tracks which user made the change and when. Requires ADMIN role or ' +
					'higher.',
				responses: {
					'200': {
						content: {
