 ...
 *
 * Extracted from the password-change guard plugin so that services that need to
 * invalidate the cache (e.g. auth/authCore after a successful password change)
 * can do so without importing from the plugins layer — which previously created
 * a circular dependency: userService → userCrud → authCore → passwordChangeGuard → userService.
 */
