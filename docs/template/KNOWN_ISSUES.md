...
This is the living list of known limitations and deferrals for the active v3 template. Keep
entries blunt, factual, and tied to the current codebase.

If something here turns into a real-world failure, escalate it into tracked work and fix it.
...
**Why this is currently acceptable.** The current file-level scan passes across every page in the template and catches the real failure modes that motivated the check (missing confirmation on delete/reset/disable handlers). No false-negatives have been observed in template or derived-app crawltest sweeps. The AST upgrade is a precision improvement unless a real false-negative appears.
