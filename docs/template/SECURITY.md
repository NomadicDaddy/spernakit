...
- **Air-gapped scope**: No multi-tenant SaaS infrastructure
...
- **Validated deployment config**: Infrastructure and secrets use JSON configuration

### Air-Gapped Scope

Spernakit keeps deployment security explicit and locally managed:

- JWT audience and issuer values derive from the configured frontend and backend URLs
- Key rotation follows the documented manual procedure
- Security configuration remains available without external control-plane services
...
- Use crypto-secure random: 32 bytes (256 bits) encoded as a hex string
...
These services are ready to use; integrate them at the points described below:
...
Use this checklist to track operational security work in your issue tracker.
