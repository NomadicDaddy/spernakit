...
Enterprise applications need flexible configuration for:
...
1. **Bun Compatibility**: Bun doesn't auto-load `.env` files (has `env = false`). Explicit JSON loading is natural fit. No need to fight Bun's design.
...
5. **Priority Chain** (Environment > JSON > Defaults):
    - Environment variables override JSON config (perfect for secrets)
    - JSON config provides defaults (perfect for development)
