...
The global limiter exempts a small set of path prefixes from counting against the 600/15min budget. Over a 30-day window, the exempt-prefix list flipped four times, twice on the same day, with two of the flips happening silently inside version-bump commits. The root cause was the absence of a written policy: each agent who touched the list re-argued the tradeoff from scratch, and the SPA's polling of `/auth/me` on every navigation kept producing false session-expiry redirects whenever the endpoint was pulled back under the global limiter.
