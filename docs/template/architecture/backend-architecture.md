...
Plugins register in order and run as an onion-model middleware stack:
...
Route modules are split by feature domain. Each one delegates to a dedicated service, and services are the only layer that touches the database.
