...
| Expected   | 4xx from provider-error path (GitHub returns `bad_verification_code`); handled with a user-facing error toast |
| Result     | _pending_                                                                                                     |
...
```
OAuth providers exercised end-to-end in Spernakit vX.Y.Z on YYYY-MM-DD; see
