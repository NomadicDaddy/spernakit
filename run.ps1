...
foreach ($currentAction in $Action) {
	...
	foreach ($dir in $Dirs) {
		...
		try {
			...
			# Detect if this is a smoke command that needs PowerShell execution
			$isSmokeCommand = $currentAction -like 'smoke:*' -or $currentAction -eq 'qc' -or $currentAction -eq 'reset' -or $currentAction -eq 'test'

			# Execute command with appropriate shell
			if ($isSmokeCommand) {
				# Use Invoke-Expression for smoke commands to properly handle PowerShell commands
				Invoke-Expression $bunCommand
				$exitCode = $LASTEXITCODE
			} else {
				# Use cmd for regular bun commands
				& cmd /c $bunCommand
				$exitCode = $LASTEXITCODE
			}
