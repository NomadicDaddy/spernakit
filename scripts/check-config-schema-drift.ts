 ...
 *
 * Regenerates the JSON schema from the Zod source in-memory and diffs it
 ...
 *
 * Without this check, the Zod source can evolve (new fields, new constraints)
 * and the committed artifact silently lags behind — exactly what happened in
 * the March/April stale-schema drift sweep: Zod had `busyTimeoutMs` and
...
function main(): void {
	...
	if (JSON.stringify(canonicalize(committed)) === JSON.stringify(canonicalize(expected))) {
		console.log('[OK] Config schema artifact is in sync with Zod source.');
		process.exit(0);
	}

	console.error(
		'[FAIL] Config schema artifact is out of sync with Zod source.\n' +
			`       File: ${schemaPath}\n` +
			'       Zod source files under backend/src/config/configSchemas/ have\n' +
