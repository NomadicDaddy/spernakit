...
// Every number below comes from a dev build, always. webVitals.ts only emits the
// `[Web Vitals]` console lines this analysis reads under `import.meta.env.DEV`, and
// dev additionally sets reportAllChanges for CLS/INP — so those two are the worst
// intermediate value seen, not the Core Web Vitals definitions (INP p98, CLS session
// max). Treat them as a relative dev-time smoke signal, never as production evidence.
console.log(`   Build measured: ${report.servedBuild ?? 'unstamped (assume dev)'}`);

if (report.webVitals.length === 0) {
	console.log('\n⚠ No Web Vitals entries in crawltest.json');
	if (report.servedBuild === 'production') {
		console.log('  Expected: console capture is dev-only. Use stored field vitals instead.');
	}
	process.exit(0);
}

console.log('   ⚠ Dev-build values. CLS/INP are worst-intermediate, not CWV definitions.');
