 ...
 * mutate settings (e.g. switching the default layout mode).  A direct DB
 * write avoids the cascading side-effects that the settings API causes
 * (e.g. /settings/user 500s).
 */
function resetLayoutDefaults(rootDir: string): void {
	try {
		...
		db.close();
		console.log('   ♻️  Layout defaults reset via DB (sidebar)');
	} catch {
