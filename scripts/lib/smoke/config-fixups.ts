...
function writeDockerConfigOverrides(
	...
	try {
		const cfg = JSON.parse(readFileSync(configPath, 'utf-8'));
		if (cfg.rateLimit) {
			const isProductionImageTest = mode === 'docker-prod';
			cfg.rateLimit.enabled = isProductionImageTest;
			cfg.rateLimit.authEnabled = isProductionImageTest;
			if (isProductionImageTest) {
				cfg.rateLimit.backend = 'memory';
				cfg.rateLimit.maxRequests = 100_000;
			}
		}
		if (mode === 'docker-prod' && cfg.server) {
			// Exercise the production image with test-only credentials and structured stdout.
		...
		// Non-fatal — the container startup will report an invalid config
	}
}
