...
import { runCommand, showCacheStatus, type Step } from './lib/smoke/run-step.ts';
...
async function main(): Promise<void> {
	...
	if (modeKey === 'docker-prod' || modeKey === 'docker-local') {
		ensureDockerTestDirs(projectRoot);
	}

	// Handle --cache-status flag
	if (cacheStatus) {
		await showCacheStatus(projectRoot, modeConfig.steps);
		return;
	}
